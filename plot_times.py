...
def fetch_latest_private_leaderboard(user_id):
    ...
    url = f"https://adventofcode.com/{YEAR}/leaderboard/private/view/{user_id}.json"
    auth = {"session": session_cookie}
    r = requests.get(url, cookies=auth)
...
def time_elapsed(ts):
    ...
    return (
        (ds.hour - start.hour) * 3600
        + (ds.minute - start.minute) * 60
        + (ds.second - start.second)
    )
