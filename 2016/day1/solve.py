...
def solve_part2(input_):
    ...
    for instruction in input_:
        ...
        if heading == 'north':
            visited_now = [(current_position[0], current_position[1] + i + 1) for i in range(blocks)]
            current_position[1] += blocks
        if heading == 'south':
            visited_now = [(current_position[0], current_position[1] - (i + 1)) for i in range(blocks)]
            current_position[1] -= blocks
        if heading == 'west':
            visited_now = [(current_position[0] - (i + 1), current_position[1]) for i in range(blocks)]
            current_position[0] -= blocks
        if heading == 'east':
            visited_now = [(current_position[0] + (i + 1), current_position[1]) for i in range(blocks)]
            current_position[0] += blocks
